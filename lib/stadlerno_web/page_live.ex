...
defmodule StadlernoWeb.PageLive do
  ...
  def handle_params(params, uri, socket) do
    ...
    new_sock =
      assign(socket, post: post, search_result: [], host: host)
      |> generate_nodes()
 ...
 def handle_event("send_message", message, socket) do
  ...
  defp generate_nodes(socket) do
        ...
        if node.id == socket.assigns.post.id do
          %{
            data: %{
              ...
              id: "#{node.id}",
              href: "#{socket.assigns.host}/post/#{node.id}"
            }
          }
        else
          %{
    ...
    push_event(socket, "pushEventToJs", %{elements: nodes  ++ tags ++ tag_links ++ links})
  end
  ...
  def render(assigns) do
    ...
    <div class="flex-1 lg:flex-none">
      <div class="form-control">
          ...
          <ul tabindex="0" class="mb-5 px-5 relative  shadow-sm menu dropdown-content bg-base-100 rounded-box w-full">
